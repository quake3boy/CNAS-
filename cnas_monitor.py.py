...
def send_training_notice_email(training_notices):
    ...
    for i, notice in enumerate(training_notices, 1):
        ...
        # 内容摘要部分（完整显示所有OCR识别内容，保留原始排版）
        summary_html = ""
        if detail.get('pdf_summary'):
            summary_html = f"""
            <div style="background:#f8fafc;border:1px solid #cbd5e1;border-radius:6px;padding:16px 20px;margin-top:10px;font-size:13px;color:#1e293b;line-height:1.9;">
                <div style="font-weight:bold;color:#0369a1;margin-bottom:12px;font-size:14px;">📄 PDF完整内容（OCR识别，共{len(detail['pdf_summary'])}字）</div>
                <div style="white-space:pre-wrap;word-break:break-all;font-family:'Courier New','Microsoft YaHei',monospace;letter-spacing:0.3px;">{detail['pdf_summary']}</div>
            </div>
            """
