...
# -*- coding: utf-8 -*-
"""
CNAS网站最新通知监控脚本 - 每小时监控版
运行频率：北京时间 8:30~18:30 每小时一次（由yml cron控制）
邮件逻辑：
  - 白天每小时运行时，有新培训通知才发实时通知邮件，无新通知静默
  - 18:30末班必发每日总结邮件（汇总全天检查情况，无论有没有新通知）
  - 异常时立即发红色紧急报警邮件
"""
import requests
...
def send_daily_summary_email(today_check_count, today_training_count=0, training_notices=None, error_msg=None):
    ...
    if has_error:
        ...
        subject = f"🔵【CNAS每日总结】今日发现{today_training_count}条新培训通知 - {today}"
    else:
    ...
    <div style="font-family:'Microsoft YaHei',Arial,sans-serif;max-width:720px;margin:0 auto;padding:20px;color:#333;">
        ...
        <div style="background:#f9fafb;padding:25px 30px;border-radius:0 0 8px 8px;border:1px solid #e5e7eb;border-top:none;">
            ...
            <div style="margin-top:25px;padding-top:15px;border-top:1px solid #e5e7eb;font-size:12px;color:#9ca3af;text-align:center;">
                本邮件由CNAS通知监控系统自动发送<br>每日北京时间 08:30~18:30 每小时监控，18:30发送每日总结<br>
    ...
    try:
        ...
        if not new_notices:
            ...
            if training_notices:
                ...
                current_training_notices = training_notices
                # 白天每小时运行时，有新通知立即发实时通知邮件
        ...
        # 如果是末班异常，也尝试发含错误信息的总结邮件
        beijing_now = get_beijing_now()
        is_last_run = (beijing_now.hour > LAST_RUN_HOUR) or \
                      (beijing_now.hour == LAST_RUN_HOUR and beijing_now.minute >= LAST_RUN_MINUTE)
        if is_last_run and state.get("last_summary_date") != today:
            print("\n=== 末班异常，尝试发送含错误信息的总结邮件 ===")
            try:
                today_training_list = state.get("today_training_list", [])
                if send_daily_summary_email(
