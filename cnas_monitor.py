...
# -*- coding: utf-8 -*-
"""
CNAS网站最新通知监控脚本 - 每日18:30版
运行频率：每天北京时间18:30一次（由yml cron控制）
每次运行都会发送每日总结邮件，有新通知时一并附上
"""
import requests
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                wait_time = attempt * 5  # 5秒, 10秒递增等待
...
def send_daily_summary_email(today_check_count, training_notices=None, error_msg=None):
    """发送每日总结邮件（每次运行必发，有新通知时一并附上）"""
    today = get_beijing_now().strftime("%Y-%m-%d")
    now_time = get_beijing_now().strftime("%H:%M:%S")
    ...
    # 邮件标题根据情况变化
    if has_error:
        subject = f"🔴【CNAS每日总结】监控异常 - {today}"
    elif has_training:
        subject = f"🔵【CNAS每日总结】发现{len(training_notices)}条新培训通知 - {today}"
    else:
        subject = f"【CNAS每日总结】今日无新培训通知 - {today}"
    ...
    if has_error:
        ...
        <div style="background:#fee2e2;border:2px solid #f87171;color:#991b1b;padding:16px 20px;border-radius:8px;margin-bottom:20px;">
            <div style="font-size:16px;font-weight:bold;margin-bottom:8px;">⚠️ 监控执行异常</div>
            <p style="margin:0;font-size:14px;">本次监控检查未能正常完成，请查看下方错误详情。</p>
        </div>
        """
    elif has_training:
        ...
        <div style="background:#dbeafe;color:#1e40af;padding:12px 16px;border-radius:6px;margin-bottom:20px;font-weight:bold;">
            🔵 检测到 <strong>{len(training_notices)}</strong> 条新的培训通知，详情见下方
        </div>
        """
    else:
    ...
    if has_training:
        ...
        <div style="margin-top:20px;">
            ...
            {build_training_notice_html(training_notices)}
        </div>
        """
    ...
    <div style="font-family:'Microsoft YaHei',Arial,sans-serif;max-width:720px;margin:0 auto;padding:20px;color:#333;">
        ...
        <div style="background:#f9fafb;padding:25px 30px;border-radius:0 0 8px 8px;border:1px solid #e5e7eb;border-top:none;">
            ...
            <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:16px 20px;margin-bottom:20px;">
                ...
                <div style="display:flex;justify-content:space-between;padding:6px 0;font-size:14px;border-bottom:1px solid #f3f4f6;">
                    <span style="color:#6b7280;">执行时间</span><span style="font-weight:bold;">{now_time} (北京时间)</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:6px 0;font-size:14px;border-bottom:1px solid #f3f4f6;">
                ...
                <div style="display:flex;justify-content:space-between;padding:6px 0;font-size:14px;">
                    ...
                    <span style="font-weight:bold;color:{'#059669' if not has_training else '#2563eb'};">
                        {'无' if not has_training else str(len(training_notices)) + ' 条'}
                    </span>
                </div>
            </div>
            ...
            <div style="margin-top:25px;padding-top:15px;border-top:1px solid #e5e7eb;font-size:12px;color:#9ca3af;text-align:center;">
                本邮件由CNAS通知监控系统自动发送<br>每日北京时间 18:30 执行并发送总结邮件<br>
                有新培训通知时直接附在邮件中，PDF原版图片+OCR文字一并送达
            </div>
        </div>
    </div>
    """
    ...
    try:
        ...
        if not new_notices:
            ...
            if training_notices:
                ...
                today_training_notices = training_notices
        ...
        # 【关键修复】每次运行都发送每日总结邮件，无论有没有新通知
        print("\n=== 发送每日总结邮件 ===")
        if send_daily_summary_email(
            today_check_count=state["today_check_count"],
            training_notices=today_training_notices if today_training_notices else None,
            error_msg=None
        ):
        ...
        # 【关键修复】异常时也尝试发送总结邮件（包含错误信息）
        print("\n=== 检测到异常，发送含错误信息的每日总结邮件 ===")
        try:
